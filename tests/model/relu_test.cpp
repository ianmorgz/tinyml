...
TEST(Relu, Shape) {
    ...
    EXPECT_EQ(relu.infer_output_shape({3, 2})[0], 3);
    EXPECT_EQ(relu.infer_output_shape({3, 2})[1], 2);
}

TEST(Relu, Forward) {
    const tinyml::model::Relu relu;
    const tinyml::tensor::Tensor<float> i1(tinyml::core::Shape({3}), 2.0f);
    const tinyml::tensor::Tensor<float> i2(tinyml::core::Shape({3}), -4.0f);
    tinyml::tensor::Tensor<float> o(tinyml::core::Shape({3}), 1.0f);
    ...
    const tinyml::tensor::Tensor<float> i3(tinyml::core::Shape({4}), 2.0f);
